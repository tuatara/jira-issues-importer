...
class Project:
    ...
    def __init__(self, name):
        self.name = name
        self._project = {'Milestones': defaultdict(int), 'Components': defaultdict(int), 'Labels': defaultdict(int),
                         'Issues': []}
    ...
    def _append_item_to_project(self, item):
        ...
        body = self._htmlentitydecode(item.description.text) + '\n<i>' +\
            item.title.text[0:item.title.text.index("]") + 1] + ' created by ' +\
            item.reporter.get('username') + '</i>'
                ...
                for outwardlink in issuelinktype.outwardlinks:
                    for issuelink in outwardlink.issuelink:
                        for issuekey in issuelink.issuekey:
                            self._project['Issues'][-1][outwardlink.get("description").replace(' ', '-')]\
                                .append(issuekey.text)
        except AttributeError:
                pass
        except KeyError:
            ...
            for issuelinktype in item.issuelinks.issuelinktype:
                for inwardlink in issuelinktype.inwardlinks:
                    for issuelink in inwardlink.issuelink:
                        for issuekey in issuelink.issuekey:
                            self._project['Issues'][-1][inwardlink.get("description").replace(' ', '-')]\
                                .append(issuekey.text)
        except AttributeError:
                pass
    ...
    def _htmlentitydecode(self, s):
        ...
        return re.sub('&(%s);' % '|'.join(name2codepoint),
            lambda m: unichr(name2codepoint[m.group(1)]), s)  # noqa: F821
