...
class Importer:
    ...
    def import_milestones(self):
        ...
        for mkey in self.project.get_milestones().iterkeys():
            data = {'title': mkey}
            r = requests.post(milestone_url, json=data, headers=self.options.headers,
            ...
            if r.status_code == 201:
                ...
                if r.status_code == 422:  # already exists
                    ms = requests.get(milestone_url + '?state=open', headers=self.options.headers,
                                      timeout=Importer._DEFAULT_TIME_OUT).json()
                    ms += requests.get(milestone_url + '?state=closed', headers=self.options.headers,
                                       timeout=Importer._DEFAULT_TIME_OUT).json()
                    f = False
                    for m in ms:
                        if m['title'] == mkey:
    ...
    def upload_github_issue(self, issue, comments):
        ...
        response = requests.post(issue_url, json=issue_data, headers=self.options.headers,
                                 timeout=Importer._DEFAULT_TIME_OUT)
    ...
    def convert_relationships_to_comments(self, issue):
        ...
        for split_item in split:
            issue['comments'].append({"body": "Is split by: " + self._replace_jira_with_github_id(blocked_item)})

        for cloned_item in cloned:
            issue['comments'].append({"body": "Is cloned by: " + self._replace_jira_with_github_id(blocked_item)})
    ...
    def _replace_github_id_placholder(self, text):
        result = text
        pattern = Importer._PLACEHOLDER_PREFIX + Importer._GITHUB_ISSUE_PREFIX + r'(\d+)' +\
            Importer._PLACEHOLDER_SUFFIX
    ...
    def _patch_comment(self, url, body):
        ...
        # print(patch_data)
        response = requests.patch(url, json=patch_data, headers=self.options.headers,
                                  timeout=Importer._DEFAULT_TIME_OUT)
        if response.status_code != 200:
            raise RuntimeError(
                "Failed to patch comment {} due to unexpected HTTP status code: {} ; text: {}"
